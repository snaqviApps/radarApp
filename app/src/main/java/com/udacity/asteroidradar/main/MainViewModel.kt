...
enum class RadarApiStatus { LAODING, ERROR, DONE }
class MainViewModel(val database: AsteroidDao,
    ...
    private val _availableAsteroid = database.getLatestAsteroid()       // reads from database directly, not being used as of now
    ...
    private fun getAsteroidsProperties() {
        viewModelScope.launch {
            _status.value = RadarApiStatus.LAODING
            AsteroidApi.retrofitService.getAsteroids(
                    start_date = "2017-09-11",          // move it to today's date
                    end_date = "2017-09-17",
                    ...
                    .enqueue(object : Callback<String> {
                        ...
                        try {
                            val pictureResult = PictureOfTheDayApi.retrofitPicOfTheDayService.getPictureOfTheDay(BuildConfig.NASA_API_KEY)
